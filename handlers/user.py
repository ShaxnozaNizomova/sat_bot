...
def _normalize_phone(text: str) -> str:
    # Very simple cleanup: keep digits and plus
    t = text.strip()
    t = re.sub(r"[^\d+]", "", t)
    return t
...
async def handle_contact(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    """
    Accepts either a Telegram contact OR a manually typed phone number.
    """
    if update.effective_user is None or update.message is None:
        return ConversationHandler.END
    ...
    # Basic validation (Uzbek numbers typically 9-13 digits, often +998...)
    ...
    if videos:
        ...
        await update.message.reply_text(
            "Registration successful! Choose a video below.",
            reply_markup=reply_markup,
        )
    else:
        await update.message.reply_text(
...
async def handle_video_selection(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if not existing_user:
        # If user isn't registered, guide them
    ...
    video = await asyncio.to_thread(get_video_by_title, title)
    if not video:
        # If user typed something else, show menu again
        await _send_video_menu(update, "Please choose a video from the buttons below:")
        return
...
registration_handler = ConversationHandler(
    ...
    states={
        ...
        # Accept contact OR typed text phone
        PHONE: [
